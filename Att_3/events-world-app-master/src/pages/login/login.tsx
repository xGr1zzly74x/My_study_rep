...
import { change_auth, change_theme, theme_selector, auth_selector}  from "../../store/slice_login"
...
const Login = () => {
  ...
  const redux_auth = useSelector(auth_selector.get_auth)//Глобальная тема
  ...
  const [auth, setauth] = useState<boolean>(redux_auth)//Локальный признак авторизации
  ...
  useEffect(() => {
    ...
    const exit_auth_e = document.querySelector('#exit_auth') as HTMLElement
    const SucAuth_e = document.querySelector('#SucAuth') as HTMLElement
    const container_e = document.querySelector('#container') as HTMLElement
    ...
    if(auth){
      ...
      exit_auth_e.style.display = 'none'
    }
