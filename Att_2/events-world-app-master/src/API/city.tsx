  ...
  endpoints: (build) => ({
    GetCityData: build.query({
      async queryFn(arg, api, extraOptions, fetchWithBQ) {
        ...
        result_coord = await fetchWithBQ(
          `//geocode-maps.yandex.ru/1.x/?apikey=${api_key_yandex}&geocode=${arg.city}&format=json`
        )
        ...
        if (result_coord.error) {
          ...
          if (str) {
            ...
            result_pol = await fetchWithBQ(
              `https://air-quality-api.open-meteo.com/v1/air-quality?latitude=${coordinates[1]}&longitude=${coordinates[0]}&hourly=pm10,pm2_5`
            )
            if (result_pol.error) {
